...
const DEEPLINK_SETTINGS_NAMESPACE = settingsNamespace('dsh-deeplink');
...
export function apply(ctx) {
  ...
  // Official settings-namespace registration (same helper as agent-loop /
  // bash-local): makes 插件配置 enumerate 'dsh-deeplink' and dispatch the card
  // registered into settings.plugin.item with that key. The toggles themselves
  // live in localStorage (browser behavior), so the host does not consume the
  // resolved value — these hooks are placeholders.
  installSettingsSection(ctx, DEEPLINK_SETTINGS_NAMESPACE, DEEPLINK_SETTINGS_SCHEMA, {}, {
    setSource: () => {},
    onChange: () => {},
  });
}
